...
<body>
  <div class="container">
    ...
    <div class="row row1">
                ...
                <tr>
                  ...
                  <td class="text-right">
                    <c:if test="${sessionScope.id==rvo.id }">
                      <span class="btn btn-sm btn-info up" data-no="${rvo.no }">수정</span>
                      <a href="../reply/delete.do?no=${rvo.no }&type=${tab}&cno=${vo.no}" class="btn btn-sm btn-success">삭제</a>
                    </c:if>
                  </td>
                </tr>
                <tr>
                ...
                <tr style="display: none" id="u${rvo.no }" class="updates">
	              <td>
	                <form method="post" action="../reply/update.do">
	                  <textarea rows="5" cols="70" style="float: left" name="msg">${rvo.msg }</textarea>
      ...
      <c:if test="${sessionScope.id!=null }"> <%-- 로그인해야 보여줌 --%>
        <table class="table">
          <tr>
            <td>
              <form method="post" action="../reply/insert.do">
                <textarea rows="5" cols="80" style="float: left" name="msg"></textarea>
                <input type="hidden" name="cno" value="${vo.no }">
                <input type="hidden" name="type" value="${tab }">
